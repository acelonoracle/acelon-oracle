...
// Fetches price data for a given currency pair from multiple CEX exchanges
export async function fetchCexPrice(
  from: string,
  to: string,
  exchanges?: string[],
  tradeAgeLimit?: number
): Promise<Array<{ price: number; exchangeId: string; certificate: string }>> {
  log(`👀 Fetching CEX price for ${from}-${to}`)
  ...
  const promises = configsToFetch.map((config) =>
    fetchFromCexExchange(config, from, to, tradeAgeLimit)
  )
...
// Fetches price data from a specific CEX exchange
async function fetchFromCexExchange(
